...
class SalesService{
    ...
    public function savepayment($id, $oid)
    {
        $create = new Payment;
        $create->paymentID = $id;
        $create->orderID = $oid;
    ...
    public function addNewTable(Request $req)
    {
        $create = new Table;
        $create->tableName = $req->name;
        $create->statusMeja = 0;

        return $this->dao->saveTable($create);
    }

    public function delTable(){
        $table = $this->dao->getLastTable();
        return $this->dao->deleteTable($table);
    }

    public function updateStatus($id){

        $order = $this->dao->findOrderById($id);
        $payment = $this->dao->findPaymentById($order->payment->paymentID);
        $payment->statusPayment = 1;
        foreach ($payment->order->menuorder as $menu){
            $menu->stat = 1;
            $menu->save();
